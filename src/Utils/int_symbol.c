 ...
 */
INT_Symbol_Table *INT_new_symbol_table (const char *name, int expected_size)
{
...
void INT_resize_symbol_table (INT_Symbol_Table *table)
{
     ...
     * power of two).   Set by INT_tweak_table_resize ().
     */
    if (new_hash_size < table->resize_target)
    {
#if 0
	/* DEBUG */
	fprintf (stderr, "INT_resize_symbol_table: Using resize target %i instead of %i!\n",
...
#if 0
    /* DEBUG */
    TG_timestamp("INT_resize_symbol_table (%s): new size %i\n",
...
INT_Symbol *INT_add_symbol (INT_Symbol_Table *table, int value, void *data)
{
	...
	{
	    L_punt ("INT_add_symbol, %s: cannot add '%i', already in table!",
		     table->name, value);
	}
    }
...
/* Returns a INT_Symbol structure with the desired value, or NULL
 ...
 */
void INT_tweak_table_resize (INT_Symbol_Table *table, 
